...
const EXTENSION_NAME = "trangquynhcode"
...
async function main() {
  ...
  // Delete any existing branches for trangquynhcode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating TrangQuynhCode extension to v${cleanVersion}"`
