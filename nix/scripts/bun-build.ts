import solidPlugin from "./packages/trangquynhcode/node_modules/@opentui/solid/scripts/solid-plugin"
import path from "path"
import fs from "fs"

const version = "@VERSION@"
const pkg = path.join(process.cwd(), "packages/trangquynhcode")
...
const manifestName = "trangquynhcode-assets.manifest"
...
const result = await Bun.build({
  ...
  define: {
    TRANGQUYNHCODE_VERSION: `'@VERSION@'`,
    OTUI_TREE_SITTER_WORKER_PATH: "/$bunfs/root/" + path.relative(pkg, parser).replace(/\\/g, "/"),
    TRANGQUYNHCODE_CHANNEL: "'latest'",
  },
  compile: {
    target,
    outfile: "trangquynhcode",
    autoloadBunfig: false,
    autoloadDotenv: false,
    //@ts-ignore (bun types aren't up to date)
    autoloadTsconfig: true,
    autoloadPackageJson: true,
    execArgv: ["--user-agent=trangquynhcode/" + version, "--use-system-ca", "--"],
    windows: {},
  },
})
...
const dest = path.join(pkg, "trangquynhcode-worker.js")
