...
const CLI_INSTALL_DIR: &str = ".trangquynhcode/bin";
const CLI_BINARY_NAME: &str = "trangquynhcode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("trangquynhcode-install.sh");
...
pub fn create_command(app: &tauri::AppHandle, args: &str) -> Command {
    ...
    return app
        .shell()
        .sidecar("trangquynhcode-cli")
        .unwrap()
        .args(args.split_whitespace())
        .env("TRANGQUYNHCODE_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("TRANGQUYNHCODE_CLIENT", "desktop")
