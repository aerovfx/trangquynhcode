...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@trangquynhcode-ai/app"
...
import { Splash } from "@trangquynhcode-ai/ui/logo"
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://trangquynhcode.ai/favicon-96x96.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`trangquynhcode:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        <ServerGate>
          {(data) => {
            ...
            window.__TRANGQUYNHCODE__.serverPassword = data().password ?? undefined
