import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@trangquynhcode-ai/sdk/v2"
import { SessionTurn } from "@trangquynhcode-ai/ui/session-turn"
import { SessionReview } from "@trangquynhcode-ai/ui/session-review"
import { DataProvider } from "@trangquynhcode-ai/ui/context"
import { DiffComponentProvider } from "@trangquynhcode-ai/ui/context/diff"
import { CodeComponentProvider } from "@trangquynhcode-ai/ui/context/code"
import { WorkerPoolProvider } from "@trangquynhcode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@trangquynhcode-ai/ui/logo"
import { IconButton } from "@trangquynhcode-ai/ui/icon-button"
import { ProviderIcon } from "@trangquynhcode-ai/ui/provider-icon"
import { createDefaultOptions } from "@trangquynhcode-ai/ui/pierre"
import { iife } from "@trangquynhcode-ai/util/iife"
import { Binary } from "@trangquynhcode-ai/util/binary"
import { NamedError } from "@trangquynhcode-ai/util/error"
import { DateTime } from "luxon"
import { SessionMessageRail } from "@trangquynhcode-ai/ui/session-message-rail"
import { createStore } from "solid-js/store"
import z from "zod"
import NotFound from "../[...404]"
import { Tabs } from "@trangquynhcode-ai/ui/tabs"
import { preloadMultiFileDiff, PreloadMultiFileDiffResult } from "@pierre/diffs/ssr"
import { Diff as SSRDiff } from "@trangquynhcode-ai/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@trangquynhcode-ai/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@trangquynhcode-ai/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@trangquynhcode-ai/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@trangquynhcode-ai/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/trangquynhcode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | TrangQuynhCode</Title>
              </Show>
              <Meta name="description" content="trangquynhcode - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/anomalyco/trangquynhcode"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
                                <IconButton
                                  as={"a"}
                                  href="https://trangquynhcode.ai/discord"
                                  target="_blank"
                                  icon="discord"
                                  variant="ghost"
                                />
