...
import { base64Decode, base64Encode } from "@trangquynhcode-ai/util/encode"
import { Avatar } from "@trangquynhcode-ai/ui/avatar"
import { ResizeHandle } from "@trangquynhcode-ai/ui/resize-handle"
import { Button } from "@trangquynhcode-ai/ui/button"
import { Icon } from "@trangquynhcode-ai/ui/icon"
import { IconButton } from "@trangquynhcode-ai/ui/icon-button"
import { InlineInput } from "@trangquynhcode-ai/ui/inline-input"
import { Tooltip, TooltipKeybind } from "@trangquynhcode-ai/ui/tooltip"
import { HoverCard } from "@trangquynhcode-ai/ui/hover-card"
import { DropdownMenu } from "@trangquynhcode-ai/ui/dropdown-menu"
import { Collapsible } from "@trangquynhcode-ai/ui/collapsible"
import { DiffChanges } from "@trangquynhcode-ai/ui/diff-changes"
import { Spinner } from "@trangquynhcode-ai/ui/spinner"
import { getFilename } from "@trangquynhcode-ai/util/path"
import { Session } from "@trangquynhcode-ai/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@trangquynhcode-ai/ui/toast"
import { useGlobalSDK } from "@/context/global-sdk"
import { useNotification } from "@/context/notification"
import { usePermission } from "@/context/permission"
import { Binary } from "@trangquynhcode-ai/util/binary"
import { retry } from "@trangquynhcode-ai/util/retry"

import { useDialog } from "@trangquynhcode-ai/ui/context/dialog"
import { useTheme, type ColorScheme } from "@trangquynhcode-ai/ui/theme"
...
export default function Layout(props: ParentProps) {
    ...
    async function pollUpdate() {
      ...
      if (updateAvailable && toastId === undefined) {
        toastId = showToast({
          ...
          description: `A new version of TrangQuynhCode (${version}) is now available to install.`,
          actions: [
            {
  ...
  const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
    ...
    const trangquynhcode = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
    ...
    return (
      <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
        <div class="size-full rounded overflow-clip">
          <Avatar
            fallback={name()}
            src={props.project.id === trangquynhcode ? "https://trangquynhcode.ai/favicon.svg" : props.project.icon?.url}
    ...
    return (
      <div class="flex h-full w-full overflow-hidden">
        <div class="w-16 shrink-0 bg-background-base flex flex-col items-center overflow-hidden">
          <div class="flex-1 min-h-0 w-full">
            ...
            >
              <DragDropSensors />
              <ConstrainDragXAxis />
              <div class="h-full w-full flex flex-col items-center gap-4 px-3 py-6 overflow-y-auto no-scrollbar">
                <div class="logo-icon mb-4 shrink-0" style="width: 32px; height: 32px; background: linear-gradient(135deg, var(--accent-primary), var(--accent-secondary)); border-radius: 8px; animation: pulse-glow 3s infinite alternate;"></div>
                <SortableProvider ids={layout.projects.list().map((p) => p.worktree)}>
                  <For each={layout.projects.list()}>
                    {(project) => (
                      <div class="nav-item group relative">
                        <SortableProject project={project} mobile={sidebarProps.mobile} />
                      </div>
                    )}
                  </For>
                </SortableProvider>
                ...
                >
                  <IconButton icon="plus" variant="ghost" size="large" onClick={chooseProject} class="rounded-[10px] bg-white/5 hover:bg-white/10" />
                </Tooltip>
              </div>
              <DragOverlay>
                <ProjectDragOverlay />
          ...
          <div class="shrink-0 w-full pt-3 pb-3 flex flex-col items-center gap-2">
            ...
            <Tooltip placement={sidebarProps.mobile ? "bottom" : "right"} value="Help">
              <IconButton
                ...
                variant="ghost"
                size="large"
                onClick={() => platform.openLink("https://trangquynhcode.ai/desktop-feedback")}
              />
            </Tooltip>
          </div>
        </div>
        ...
        <Show when={expanded()}>
          <div
            classList={{
              "flex flex-col min-h-0 bg-background-stronger border border-b-0 border-border-weak-base rounded-tl-sm glass-pane": true,
          ...
          >
            <Show when={project()} keyed>
              {(p) => (
                <>
                  <div class="pane-header px-4 pt-4 mb-0 flex justify-between items-center">
                    <h3 class="text-[10px] uppercase tracking-[2px] font-semibold text-text-dim">Explorer</h3>
                    <span class="badge text-[9px] px-1.5 py-0.5 rounded bg-accent-primary font-bold text-white">TRANGQUYNHCODE</span>
                  </div>
              ...
              <div class="shrink-0 px-2 py-3 border-t border-border-weak-base">
                <div class="rounded-md bg-background-base shadow-xs-border-base">
                  <div class="p-3 flex flex-col gap-2">
                    ...
                    <div class="text-text-base">Trang Quynh code includes free models so you can start immediately.</div>
  ...
  return (
    <div class="relative bg-background-base flex-1 min-h-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
      ...
      <div class="flex-1 min-h-0 flex">
        ...
        <main
          classList={{
            "size-full overflow-x-hidden flex flex-col items-start contain-strict border-t border-border-weak-base glass-pane": true,
