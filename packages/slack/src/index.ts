...
import { createTrangquynhcode, type ToolPart } from "@trangquynhcode-ai/sdk"
...
console.log("🚀 Starting trangquynhcode server...")
const trangquynhcode = await createTrangquynhcode({
  port: 0,
})
...
const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
  ; (async () => {
    const events = await trangquynhcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new trangquynhcode session...")
    const { client, server } = trangquynhcode
    ...
    console.log("✅ Created trangquynhcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to trangquynhcode:", message.text)
