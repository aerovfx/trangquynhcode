...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for trangquynhcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal trangquynhcode sessions
...
trangquynhcode acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via trangquynhcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | trangquynhcode acp
```
...
### Mapping to TrangQuynhCode

ACP sessions map cleanly to trangquynhcode's internal session model:
