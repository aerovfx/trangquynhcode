...
import type { Event, TrangquynhcodeClient, SessionMessageResponse } from "@trangquynhcode-ai/sdk/v2"
import { applyPatch } from "diff"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk: _sdk }: { sdk: TrangquynhcodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    private sdk: TrangquynhcodeClient
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `trangquynhcode auth login` in the terminal",
        name: "Login with trangquynhcode",
        id: "trangquynhcode-login",
      }
      ...
      if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
        authMethod._meta = {
          "terminal-auth": {
            command: "trangquynhcode",
            args: ["auth", "login"],
            label: "TrangQuynhCode Login",
          },
        }
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const trangquynhcodeProvider = providers.find((p) => p.id === "trangquynhcode")
    if (trangquynhcodeProvider) {
      if (trangquynhcodeProvider.models["big-pickle"]) {
        return { providerID: "trangquynhcode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(trangquynhcodeProvider.models))
      if (best) {
        return {
