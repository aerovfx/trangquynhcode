...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall trangquynhcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall TrangQuynhCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g trangquynhcode-ai",
      pnpm: "pnpm uninstall -g trangquynhcode-ai",
      bun: "bun remove -g trangquynhcode-ai",
      yarn: "yarn global remove trangquynhcode-ai",
      brew: "brew uninstall trangquynhcode",
      choco: "choco uninstall trangquynhcode",
      scoop: "scoop uninstall trangquynhcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall trangquynhcode -y -r`.quiet().nothrow()
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".trangquynhcode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using TrangQuynhCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# trangquynhcode") || content.includes(".trangquynhcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".trangquynhcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".trangquynhcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".trangquynhcode"))
    ) {
      continue
    }
