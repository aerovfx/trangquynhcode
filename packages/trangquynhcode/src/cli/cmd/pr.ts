...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run trangquynhcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for trangquynhcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found trangquynhcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`trangquynhcode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting trangquynhcode...")
        UI.println()

        // Launch trangquynhcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const trangquynhcodeArgs = sessionId ? ["-s", sessionId] : []
        const trangquynhcodeProcess = spawn("trangquynhcode", trangquynhcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          trangquynhcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`trangquynhcode exited with code ${code}`))
          })
          trangquynhcodeProcess.on("error", reject)
        })
      },
    })
  },
