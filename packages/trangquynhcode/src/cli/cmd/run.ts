...
import { createTrangquynhcodeClient, type TrangquynhcodeClient } from "@trangquynhcode-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run trangquynhcode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running trangquynhcode server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: TrangquynhcodeClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createTrangquynhcodeClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.TRANGQUYNHCODE_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createTrangquynhcodeClient({ baseUrl: "http://trangquynhcode.internal", fetch: fetchFn })
