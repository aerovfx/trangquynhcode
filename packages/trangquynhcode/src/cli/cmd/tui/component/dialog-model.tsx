...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "trangquynhcode" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
...
export function DialogModel(props: { providerID?: string }) {
  ...
  const options = createMemo(() => {
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "trangquynhcode" && model.id.includes("-nano"),
              footer: model.cost?.input === 0 && provider.id === "trangquynhcode" ? "Free" : undefined,
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
    ...
    const providerOptions = pipe(
      sync.data.provider,
      sortBy(
        (provider) => provider.id !== "trangquynhcode",
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "trangquynhcode" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "trangquynhcode" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
