...
test("loads JSON config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "trangquynhcode.json"),
        JSON.stringify({
...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "trangquynhcode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://trangquynhcode.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "trangquynhcode.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "trangquynhcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .trangquynhcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const trangquynhcodeDir = path.join(dir, ".trangquynhcode")
      await fs.mkdir(trangquynhcodeDir, { recursive: true })
      const agentDir = path.join(trangquynhcodeDir, "agent")
...
test("loads agents from .trangquynhcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const trangquynhcodeDir = path.join(dir, ".trangquynhcode")
      await fs.mkdir(trangquynhcodeDir, { recursive: true })

      const agentsDir = path.join(trangquynhcodeDir, "agents")
...
test("loads commands from .trangquynhcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const trangquynhcodeDir = path.join(dir, ".trangquynhcode")
      await fs.mkdir(trangquynhcodeDir, { recursive: true })

      const commandDir = path.join(trangquynhcodeDir, "command")
...
test("loads commands from .trangquynhcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const trangquynhcodeDir = path.join(dir, ".trangquynhcode")
      await fs.mkdir(trangquynhcodeDir, { recursive: true })

      const commandsDir = path.join(trangquynhcodeDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "trangquynhcode.json"),
        JSON.stringify({ $schema: "https://trangquynhcode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "trangquynhcode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .trangquynhcode config
      const projectDir = path.join(dir, "project")
      const trangquynhcodeDir = path.join(projectDir, ".trangquynhcode")
      await fs.mkdir(trangquynhcodeDir, { recursive: true })
      ...
      // Local .trangquynhcode config with different plugins
      await Bun.write(
        path.join(trangquynhcodeDir, "trangquynhcode.json"),
        JSON.stringify({
          $schema: "https://trangquynhcode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .trangquynhcode config with some overlapping plugins
      await Bun.write(
        path.join(trangquynhcodeDir, "trangquynhcode.json"),
        JSON.stringify({
          $schema: "https://trangquynhcode.ai/config.json",
...
test("local .trangquynhcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .trangquynhcode directory config enables it
      const trangquynhcodeDir = path.join(dir, ".trangquynhcode")
      await fs.mkdir(trangquynhcodeDir, { recursive: true })
      await Bun.write(
        path.join(trangquynhcodeDir, "trangquynhcode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/trangquynhcode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/trangquynhcode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-trangquynhcode@2.4.3")).toBe("oh-my-trangquynhcode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@trangquynhcode/plugin@2.0.0")).toBe("@trangquynhcode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-trangquynhcode@2.4.3", "file:///project/.trangquynhcode/plugin/oh-my-trangquynhcode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.trangquynhcode/plugin/oh-my-trangquynhcode.js")
  })
  ...
  test("local plugin directory overrides global trangquynhcode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const trangquynhcodeDir = path.join(projectDir, ".trangquynhcode")
        const pluginDir = path.join(trangquynhcodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
