...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const trangquynhcodeModel = {
      ...openaiModel,
      providerID: "trangquynhcode",
      api: {
        id: "trangquynhcode-test",
        url: "https://api.trangquynhcode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, trangquynhcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.trangquynhcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.trangquynhcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          trangquynhcode: { itemId: "msg_trangquynhcode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              trangquynhcode: { itemId: "msg_trangquynhcode_part" },
    ...
    expect(result[0].providerOptions?.trangquynhcode?.itemId).toBe("msg_trangquynhcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.trangquynhcode?.itemId).toBe("msg_trangquynhcode_part")
