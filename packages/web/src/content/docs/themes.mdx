...
With TrangQuynhCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, TrangQuynhCode uses our own `trangquynhcode` theme.
...
TrangQuynhCode comes with several built-in themes.
...
- Want TrangQuynhCode to match their terminal's appearance
...
TrangQuynhCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/trangquynhcode/themes/*.json` or `$XDG_CONFIG_HOME/trangquynhcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.trangquynhcode/themes/*.json`
4. **Current working directory** - Defined in `./.trangquynhcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/trangquynhcode/themes
vim ~/.config/trangquynhcode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .trangquynhcode/themes
vim .trangquynhcode/themes/my-theme.json
```
...
{
  "$schema": "https://trangquynhcode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
