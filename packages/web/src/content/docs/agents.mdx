...
There are two types of agents in TrangQuynhCode; primary agents and subagents.
...
TrangQuynhCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
TrangQuynhCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

TrangQuynhCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `trangquynhcode.json` config file:
...
- Global: `~/.config/trangquynhcode/agents/`
- Per-project: `.trangquynhcode/agents/`

```markdown title="~/.config/trangquynhcode/agents/review.md"
---
...
If no temperature is specified, TrangQuynhCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global TrangQuynhCode config and the project specific config.
...
The model ID in your TrangQuynhCode config uses the format `provider/model-id`. For example, if you're using [TrangQuynhCode Zen](/docs/zen), you would use `trangquynhcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="trangquynhcode.json" {3-6,9-12}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="trangquynhcode.json" {3-5,8-10}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="trangquynhcode.json" {8}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "agent": {
    "build": {
...
```json title="trangquynhcode.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `trangquynhcode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/trangquynhcode).
:::
...
```markdown title="~/.config/trangquynhcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/trangquynhcode/agents/security-auditor.md"
---
