...
description: What's new in TrangQuynhCode 1.0.
---

TrangQuynhCode 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same trangquynhcode server.
...
version. However some older versions of TrangQuynhCode always grab latest.

To upgrade manually, run

```bash
$ trangquynhcode upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ trangquynhcode upgrade 0.15.31
```
