---
title: Server
description: Interact with trangquynhcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `trangquynhcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an trangquynhcode client can use.
...
```bash
trangquynhcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
trangquynhcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `TRANGQUYNHCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `trangquynhcode`, or set `TRANGQUYNHCODE_SERVER_USERNAME` to override it. This applies to both `trangquynhcode serve` and `trangquynhcode web`.

```bash
TRANGQUYNHCODE_SERVER_PASSWORD=your-password trangquynhcode serve
```
...
When you run `trangquynhcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the trangquynhcode server to interact with trangquynhcode programmatically.
:::

This architecture lets trangquynhcode support multiple clients and allows you to interact with trangquynhcode programmatically.

You can run `trangquynhcode serve` to start a standalone server. If you have the
trangquynhcode TUI running, `trangquynhcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the TrangQuynhCode [IDE](/docs/ide) plugins.
...
The trangquynhcode server exposes the following APIs.
