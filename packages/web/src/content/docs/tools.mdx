...
Tools allow the LLM to perform actions in your codebase. TrangQuynhCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in TrangQuynhCode.
...
```json title="trangquynhcode.json" {4}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "permission": {
    "bash": "allow"
...
:::note
This tool is only available when `TRANGQUYNHCODE_EXPERIMENTAL_LSP_TOOL=true` (or `TRANGQUYNHCODE_EXPERIMENTAL=true`).
:::
