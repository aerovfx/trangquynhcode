---
title: Providers
description: Using any LLM provider in TrangQuynhCode.
---

import config from "../../../config.mjs"
export const console = config.console

TrangQuynhCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your TrangQuynhCode config.
...
in `~/.local/share/trangquynhcode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your TrangQuynhCode
config.
...
```json title="trangquynhcode.json" {6}
{
...
TrangQuynhCode Zen is a list of models provided by the TrangQuynhCode team that have been
tested and verified to work well with TrangQuynhCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with TrangQuynhCode Zen.
:::

1. Run the `/connect` command in the TUI, select trangquynhcode, and head to [trangquynhcode.ai/auth](https://trangquynhcode.ai/auth).
...
It works like any other provider in TrangQuynhCode and is completely optional to use.
...
To use Amazon Bedrock with TrangQuynhCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running trangquynhcode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY trangquynhcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile trangquynhcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX trangquynhcode
   ```
   ...
   For project-specific or persistent configuration, use `trangquynhcode.json`:

   ```json title="trangquynhcode.json"
   {
     "$schema": "https://trangquynhcode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for trangquynhcode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX trangquynhcode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX trangquynhcode
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your trangquynhcode config.
...
2. Choose your authentication method:
   ...
   3. Name: `TrangQuynhCode`, Scopes: `api`
...
```bash
GITLAB_INSTANCE_URL=https://gitlab.company.com GITLAB_TOKEN=glpat-xxxxxxxxxxxxxxxxxxxx trangquynhcode
```
...
Customize through `trangquynhcode.json`:
...
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "plugin": ["@gitlab/trangquynhcode-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with trangquynhcode:
...
To use Google Vertex AI with TrangQuynhCode:
...
2. Set the required environment variables:
   ...
   Set them while running trangquynhcode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id trangquynhcode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through trangquynhcode, you can always configure it yourself.
...
```jsonc title="~/.config/trangquynhcode/trangquynhcode.jsonc"
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "provider": {
    "helicone": {
    ...
    "helicone": {
      ...
      "options": {
        ...
        "headers": {
          ...
          "Helicone-User-Id": "trangquynhcode",
        },
      },
    },
  },
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [trangquynhcode-helicone-session](https://github.com/H2Shami/trangquynhcode-helicone-session) plugin to automatically log each TrangQuynhCode conversation as a session in Helicone.

```bash
npm install -g trangquynhcode-helicone-session
```
...
```json title="trangquynhcode.json"
{
  "plugin": ["trangquynhcode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each TrangQuynhCode conversation listed as a separate session.
...
You can configure trangquynhcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="trangquynhcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure trangquynhcode to use local models through LM Studio.

```json title="trangquynhcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure trangquynhcode to use local models through Ollama.

```json title="trangquynhcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with TrangQuynhCode:
...
3. Copy the API key for use in TrangQuynhCode.
...
6. **Important**: Before using cloud models in TrangQuynhCode, you must pull the model information locally:
...
TrangQuynhCode Zen is a list of tested and verified models provided by the TrangQuynhCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>TrangQuynhCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **TrangQuynhCode Zen**.

   ```txt
   /connect
   ```

3. Enter your TrangQuynhCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your trangquynhcode config.
...
5. You can also customize them through your trangquynhcode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' trangquynhcode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group trangquynhcode
   ```
...
You can also customize models through your trangquynhcode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with trangquynhcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in trangquynhcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `trangquynhcode.json` file in your project directory:

   ```json title="trangquynhcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://trangquynhcode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="trangquynhcode.json" {9,11,17-20}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow TrangQuynhCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `trangquynhcode auth list` to see if the credentials
...
2. For custom providers, check the trangquynhcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your trangquynhcode config.
