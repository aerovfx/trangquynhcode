...
You can add external tools to TrangQuynhCode using the _Model Context Protocol_, or MCP. TrangQuynhCode supports both local and remote servers.
...
You can define MCP servers in your [TrangQuynhCode Config](https://trangquynhcode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="trangquynhcode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/trangquynhcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="trangquynhcode.jsonc" {15}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
TrangQuynhCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, TrangQuynhCode will:
...
If the server requires authentication, TrangQuynhCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `trangquynhcode mcp auth <server-name>`.
...
```json title="trangquynhcode.json" {7-11}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
trangquynhcode mcp auth my-oauth-server
```
...
```bash
trangquynhcode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, TrangQuynhCode will store the tokens securely in `~/.local/share/trangquynhcode/mcp-auth.json`.
...
```json title="trangquynhcode.json" {7}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "my-api-key-server": {
...
trangquynhcode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in TrangQuynhCode, alongside built-in tools. So you can manage them through the TrangQuynhCode config like any other tool.
...
```json title="trangquynhcode.json" {14}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="trangquynhcode.json" {11, 14-18}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="trangquynhcode.json" {4-8}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
trangquynhcode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect TrangQuynhCode to your Sentry account.
...
```json title="trangquynhcode.json" {4-7}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="trangquynhcode.json" {7-9}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mcp": {
    "context7": {
