...
:::caution
Modes are now configured through the `agent` option in the trangquynhcode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in trangquynhcode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the trangquynhcode config.
...
trangquynhcode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.trangquynhcode/plans/*.md` to detail the plan itself
...
Configure modes in your `trangquynhcode.json` config file:
...
- Global: `~/.config/trangquynhcode/modes/`
- Project: `.trangquynhcode/modes/`

```markdown title="~/.config/trangquynhcode/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global trangquynhcode config and the project specific config.
...
```json title="trangquynhcode.json" {4-14}
{
  "$schema": "https://trangquynhcode.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.trangquynhcode/modes/` for project-specific modes or `~/.config/trangquynhcode/modes/` for global modes:

```markdown title=".trangquynhcode/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/trangquynhcode/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
