...
TrangQuynhCode has a list of keybinds that you can customize through the TrangQuynhCode config.
...
TrangQuynhCode uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The TrangQuynhCode desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `trangquynhcode.json`.
