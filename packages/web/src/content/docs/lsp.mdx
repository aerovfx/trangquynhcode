---
title: LSP Servers
description: TrangQuynhCode integrates with your LSP servers.
---

TrangQuynhCode integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

TrangQuynhCode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `TRANGQUYNHCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When trangquynhcode opens a file, it:
...
You can customize LSP servers through the `lsp` section in your trangquynhcode config.
