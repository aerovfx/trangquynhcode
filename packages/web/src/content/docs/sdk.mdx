---
title: SDK
description: Type-safe JS client for trangquynhcode server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The trangquynhcode JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control trangquynhcode programmatically.
...
```bash
npm install @trangquynhcode-ai/sdk
```
...
Create an instance of trangquynhcode:
...
You can pass a configuration object to customize behavior. The instance still picks up your `trangquynhcode.json`, but you can override or add configuration inline:

```javascript
import { createOpencode } from "@trangquynhcode-ai/sdk"

const trangquynhcode = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${trangquynhcode.server.url}`)
...
If you already have a running instance of trangquynhcode, you can create a client instance to connect to it:

```javascript
import { createOpencodeClient } from "@trangquynhcode-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@trangquynhcode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*trangquynhcode" },
})
